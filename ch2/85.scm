...
; lowered: Begin by defining a generic operation project that “pushes” an object
...
; project it and raise the result back to the type we started with, we end up
; with something equal to what we started with. Show how to implement this idea
; in detail, by writing a drop procedure that drops an object as far as
; possible. You will need to design the various projection operations and
...
(put 'drop 'rational (lambda (x) (make-scheme-number (round (/ (numer x) (denom x))))))
