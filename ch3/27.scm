...
; stored using as keys the arguments that produced the values. When the memoized
; procedure is asked to compute a value, it first checks the table to see if the
; value is already there and, if so, just returns that value. Otherwise, it
; computes the new value in the ordinary way and stores this in the table. As an
; example of memoization, recall from Section 1.2.2 the exponential process for
...
; Every fib(n) only calculate once.
; Yes, but less efficient, because fib will call fib instead of memo-fib.
