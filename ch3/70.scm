...
; Exercise 3.56, if we define a way to say that one pair of integers is “less
; than” another. One way to do this is to define a “weighting function” W (i, j)
...
; merge-weighted takes an additional argument weight, which is a procedure that
; computes the weight of a pair, and is used to determine the order in which
; elements should appear in the resulting merged stream. Using this, generalize
; pairs to a procedure weighted-pairs that takes two streams, together with a
; procedure that computes a weighting function, and generates the stream of
; pairs, ordered according to weight. Use your procedure to generate
...
(define sa (weighted-pairs integers integers (lambda (x) (+ (car x) (cadr x)))))
