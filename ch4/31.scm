...
; incompatible change to Scheme. It might be nicer to implement lazy evaluation
; as an upward-compatible extension, that is, so that ordinary Scheme programs
...
; delayed. While we’re at it, we may as well also give the user the choice
...
; Scheme. You will have to implement new syntax procedures to handle the new
; syntax for define. You must also arrange for eval or apply to determine when
; arguments are to be delayed, and to force or delay arguments accordingly, and
