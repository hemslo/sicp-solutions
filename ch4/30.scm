...
; a sequence. Since the value of an expression in a sequence other than the last
; one is not used (the expression is there only for its effect, such as
; assigning to a variable or printing), there can be no subsequent use of this
; value (e.g., as an argument to a primitive procedure) that will cause it to be
; forced. Cy thus thinks that when evaluating sequences, we must force all
...
; original: (p1 1): (1 2), (p2 1): 1
; new: (p1 1): (1 2), (p2 1): (1 2)
...
; original is better, it's normal for lazy evaluating.
