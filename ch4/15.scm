...
; evaluating the expression (p a) returns a value (as opposed to terminating
...
; if (try try) run-forever, then (halts p p) should be false, so it's 'halted
; if (try try) halted, then (halts p p) should be true, so it's run-forever
